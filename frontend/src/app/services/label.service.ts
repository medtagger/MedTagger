...
export class LabelService {
        ...
        rawElements.forEach((element: any) => {
            ...
            switch (element.tool) {
                case 'RECTANGLE':
                    ...
                    break;
                case 'POINT':
                    elements.push(new PointSelection(element.x, element.y, element.slice_index, labelTag, SliceSelectionType.NORMAL));
                    break;
                case 'CHAIN':
                    const points: Array<Point> = [];
                    element.points.forEach(point => points.push(new Point(point.x, point.y)));
                    elements.push(new ChainSelection(points, element.loop, element.slice_index, labelTag, SliceSelectionType.NORMAL));
                    break;
                case 'BRUSH':
                    // Image for Brush will be loaded dynamically together with Slice Images
                    elements.push(new BrushSelection(undefined, element.slice_index, labelTag, SliceSelectionType.NORMAL));
                    break;
                default:
    ...
    getRandomLabel(task: Task): Promise<Label> {
        return new Promise((resolve, reject) => {
            this.http
                .get<RandomLabelResponse>(environment.API_URL + '/labels/random')
                .toPromise()
                .then(
                    response => {
                        ...
                        resolve(
                            new Label(
                                ...
                                response.scan_id,
                                response.status,
                                this.convertElements(response.elements, task),
                                response.labeling_time,
                                response.comment
                            )
                        );
                    },
    ...
    getLabelByID(labelID: string, task: Task): Promise<Label> {
        return new Promise((resolve, reject) => {
            this.http
                .get<RandomLabelResponse>(environment.API_URL + '/labels/' + labelID)
                .toPromise()
                .then(
                    response => {
    ...
    changeStatus(labelId: string, status: string) {
        return new Promise((resolve, reject) => {
            ...
            this.http.put(environment.API_URL + '/labels/' + labelId + '/status', payload).toPromise()
                .then(
                    _ => {
                        resolve();
                    },

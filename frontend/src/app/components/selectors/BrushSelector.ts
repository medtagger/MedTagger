...
export class BrushSelector extends SelectorBase<BrushSelection> implements Selector<BrushSelection> {
    ...
    drawSelection(selection: BrushSelection, color: string): any {
        ...
        const selectionLayerImage: HTMLImageElement = selection.selectionLayer;
        const requiredScale: { scaleWidth: number, scaleHeight: number } =
            this.getViewScaleDifference(selectionLayerImage.width, selectionLayerImage.height);

        console.log('BrushSelector | drawSelection | selection: ', requiredScale);

        this.canvasCtx.scale(requiredScale.scaleWidth, requiredScale.scaleHeight);
        this.canvasCtx.drawImage(selection.selectionLayer, 0, 0);
    }

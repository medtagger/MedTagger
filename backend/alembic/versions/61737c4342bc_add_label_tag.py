...
def upgrade():
    ...
    op.create_foreign_key(op.f('fk_LabelElements_tag_id_LabelTags'), 'LabelElements', 'LabelTags', ['tag_id'],
                          ['id'])
    ...
    op.execute(
        labels.update().where(labels.c.status == 'NOT_VERIFIED').values({'tmp_status': 'NOT_VERIFIED'})
    )
    op.execute(
        labels.update().where(labels.c.status == 'VALID').values({'tmp_status': 'VERIFIED'})
    )
    op.execute(
        labels.update().where(labels.c.status == 'INVALID').values({'tmp_status': 'VERIFIED'})
    )
...
def downgrade():
    ...
    op.add_column('Labels',
                  sa.Column('tmp_status', label_status_enum, server_default='NOT_VERIFIED',
                            nullable=False))
    ...
    op.execute(
        labels.update().where(labels.c.status == 'VERIFIED').values({'tmp_status': 'NOT_VERIFIED'})
    )
    op.drop_column('Labels', 'status')
    op.alter_column('Labels', 'tmp_status', new_column_name='status')

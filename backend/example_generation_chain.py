...
chain_label_elements = query.all()

ground_truth = generator.generate(chain_label_elements)
print('Ground Truth:')
pprint(ground_truth)
...
users_specificity, users_sensitivity, users_scores = compute_specificity_and_sensitivity_for_users(
    algorithm, users, chain_label_elements, ground_truth)

users_ids = set(e.label.owner_id for e in chain_label_elements)
...
figures.mean_labeling_time_vs_score(chain_label_elements, users_scores)

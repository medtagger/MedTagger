export MEDTAGGER__API_HOST="0.0.0.0"
export MEDTAGGER__API_PORT=51000
export MEDTAGGER__API_DEBUG=1
export MEDTAGGER__API_SECRET_KEY="SECRET_KEY"

export MEDTAGGER__DB_DATABASE_URI="postgresql://medtagger_user:MedTa99er!@127.0.0.1:52003/medtagger"

export MEDTAGGER__HBASE_HOST="127.0.0.1"
export MEDTAGGER__HBASE_PORT=52002
export MEDTAGGER__HBASE_REST_PORT=52001
export MEDTAGGER__HBASE_CONNECTION_POOL_SIZE=10

export MEDTAGGER__CELERY_BROKER="pyamqp://guest:guest@127.0.0.1//"


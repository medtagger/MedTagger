...
for table_name in tables_to_drop:
    if user_agrees('Do you want to drop table "{}"?'.format(table_name)):
...
for table_name in HBASE_SCHEMA:
    table = connection.table(table_name)
    if not table.exists():
        if user_agrees('Do you want to create table "{}"?'.format(table_name)):
        ...
        if columns_to_add and \
                user_agrees('Do you want to add columns {} to "{}"?'.format(columns_to_add, table_name)):
            table.add_columns(*columns_to_add)

        if columns_to_drop and \
                user_agrees('Do you want to drop columns {} to "{}"?'.format(columns_to_drop, table_name)):
